...
def hefty_hogs(player_score, opponent_score):
    ...
    while opponent_score>0:
        temp=digit_fn(opponent_score%10)(temp)
        opponent_score//=10
    return temp%30
    # END PROBLEM 2
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    return leader,"Player "+str(leader)+" takes the lead by "+str(lead_score)
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    ans=0
    for i in range(1,11):
        k=make_averaged(roll_dice,total_samples)
        num=k(i,dice)
        if num>ans:
            ans=num
            result=i
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    point=hefty_hogs(score,opponent_score)
    if point>=threshold:
        return 0
    else:
        return num_rolls
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    point=hefty_hogs(score,opponent_score)
    if point>=threshold or ((score+point)%10==opponent_score%10 and opponent_score%10!=0):
            return 0
    return num_rolls
    # END PROBLEM 11
